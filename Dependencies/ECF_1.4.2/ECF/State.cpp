...
void State::readParameters()
{
	...
	if(registry_->isModified("batch.repeats") && batchRepeats_ > 0)
		bBatchStart_ = true;
	else
		bBatchStart_ = false;
}
...
bool State::parseConfig(std::string filename) 
{
	...
	pt::ptree config;
	pt::read_xml(ss, config);

	pt::ptree ecf = config.get_child("ECF");

	BOOST_FOREACH(pt::ptree::value_type &v, ecf) {

		bool ok = true;

		if(v.first == NODE_GENOTYPE)
			ok &= parseGenotypeNode(v.second);
		else if(v.first == NODE_ALGORITHM)
			ok &= parseAlgorithmNode(v.second);
		else if(v.first == NODE_REGISTRY)
			ok &= registry_->readEntries(v.second);
		else if(v.first == NODE_POPULATION)
			continue;
		else
			std::cout << "Unknown node: " << v.first << std::endl;
 ...
 */
bool State::parseAlgorithmNode(pt::ptree& node) 
{
	int n = node.size();
	if (n > 1)
		std::cout << "Warning: multiple Algorithm nodes found! (using the first one)" << std::endl;

	// take only the first subnode
	pt::ptree::value_type v = *(node.begin());

	alg_iter alg = mAlgorithms_.find(v.first);
	if (alg == mAlgorithms_.end()) {
		throw std::string("Error: unknown Algorithm : ") + v.first;
	}

	algorithm_ = alg->second;
	bAlgorithmSet_ = true;

	if (! registry_->readEntries(v.second, v.first))
		return false;
 ...
 * \param node	current XML node
 */
bool State::parseGenotypeNode(pt::ptree& node) 
{
	BOOST_FOREACH(pt::ptree::value_type &v, node.get_child("")) {

		gen_iter gen = mGenotypes_.find(v.first);
		if (gen == mGenotypes_.end()) {
			throw std::string("Error: unknown Genotype : ") + v.first;
		}
		...
		if (! registry_->readEntries(v.second, v.first, genotypeId))
			return false;
	}
