...
bool TreeCrxSimple::mate(GenotypeP gen1, GenotypeP gen2, GenotypeP ch)
{
	...
	while(1) {
		...
		if(state_->getRandomizer()->getRandomDouble() > funcChoiceProb_ || maleNonLeafIndexes.empty()) {
			uint randomLeafIndex = state_->getRandomizer()->getRandomInteger(0 , (uint) maleLeafIndexes.size() - 1);
			mIndex = maleLeafIndexes [ randomLeafIndex ];
		}
		else {
			uint randomNonLeafIndex = state_->getRandomizer()->getRandomInteger(0 , (uint) maleNonLeafIndexes.size() - 1);
		...
		if(state_->getRandomizer()->getRandomDouble() > funcChoiceProb_ || femaleNonLeafIndexes.empty()) {
			uint randomLeafIndex = state_->getRandomizer()->getRandomInteger(0 , (uint) femaleLeafIndexes.size() - 1);
			fIndex = femaleLeafIndexes [ randomLeafIndex ];
		}
		else {
			uint randomNonLeafIndex = state_->getRandomizer()->getRandomInteger(0 , (uint) femaleNonLeafIndexes.size() - 1);
